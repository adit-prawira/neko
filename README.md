...
- **Familiar API** — REST today, gRPC scaffolded on the same port (PR #37). Curl-friendly.
...
| gRPC API | Scaffold shipped (proto + cmux + reflection); RPCs pending (PRs #35, #36) |
...
PR #37 ships the gRPC scaffold. When `neko serve` is running on the default port, both REST (HTTP/1.1) and gRPC (HTTP/2) accept connections on the same socket — cmux demuxes by protocol.
...
- **Reflection** — `grpcurl` works out of the box:
  ...
  grpcurl -plaintext localhost:3434 neko.v1.CollectionService/Create  # returns Unimplemented until PR #35 lands
  ```
