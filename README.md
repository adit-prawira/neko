...
| gRPC API | 9 collection + vector RPCs shipped (PR #38); full status mapping + parity test pending (PR #36) |
...
PR #37 ships the gRPC scaffold; PR #38 ships the 9 collection + vector RPCs. When `neko serve` is running on the default port, both REST (HTTP/1.1) and gRPC (HTTP/2) accept connections on the same socket — cmux demuxes by protocol.
