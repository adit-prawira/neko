...
- [x] Go CLI (`serve`, `create`, `insert`, `search`, `delete`, `stats`, `bench`, `config`, `version`) — current state: version, create, list, drop, insert, get, search, delete, upsert, serve, stats, bench, and config (`init` subcommand scaffolded via PR #59) are wired. CLI is split per command into `pkg/cli/<verb>_command.go`; constructors are exported as `New<Verb>Cmd` (PR #51).
