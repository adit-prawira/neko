...
int32_t neko_create(const char *name, uint32_t dim, uint8_t metric, const char *model, uint8_t index_type);
...
int32_t neko_upsert(const char *name, const char *id, const float *vector, uint32_t len, const char *metadata, uint8_t *created);
