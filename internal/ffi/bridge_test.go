...
func TestUpsertAndGet(t *testing.T) {
	...
	result, err := Get("go_test_upsert_get", "doc1", 3)
	if err != nil {
		t.Fatalf("Get failed: %v", err)
	}
	if result[0] != 1.0 || result[1] != 2.0 || result[2] != 3.0 {
...
func TestUpsertExistingReplaces(t *testing.T) {
	...
	result, err := Get("go_test_upsert_replace", "doc1", 3)
	if err != nil {
		t.Fatalf("Get failed: %v", err)
	}
	if result[0] != 0.0 || result[1] != 1.0 || result[2] != 0.0 {
		t.Errorf("vector mismatch: got [%v %v %v], want [0 1 0]", result[0], result[1], result[2])
	}
}
...
func TestUpsertWithEmptyVector(t *testing.T) {
	testInit(t)
